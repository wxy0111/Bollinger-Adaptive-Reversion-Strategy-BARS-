...
# Head-entry trend filter.
# 开仓趋势过滤：破轨时如果布林形态已经明显单边趋势化，则跳过头仓。
ENTRY_TREND_FILTER_ENABLED = True
ENTRY_TREND_FILTER_KLINES = 3
ENTRY_TREND_FILTER_SCORE_THRESHOLD = 4
ENTRY_TREND_FILTER_MID_SLOPE_PCT_PER_HOUR = 0.20
ENTRY_TREND_FILTER_EDGE_SLOPE_PCT_PER_HOUR = 0.60
ENTRY_TREND_FILTER_WIDTH_SLOPE_PCT_PER_HOUR = 20.0
ENTRY_TREND_FILTER_STRONG_BREAK_ENABLED = False
ENTRY_TREND_FILTER_STRONG_KLINES = 2
...
# Add-on risk budget.
# 补仓风险预算：补仓必须真正改善均价，否则即使价格间距满足也不补。
ADDON_RISK_BUDGET_ENABLED = True
ADDON_MIN_AVG_IMPROVE_USD = 1.0
ADDON_MIN_AVG_IMPROVE_GAP_RATIO = 0.15
